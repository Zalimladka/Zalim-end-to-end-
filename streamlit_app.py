...
if not st.session_state.logged_in:
    ...
    main_app()
st.markdown('<div class="footer">Made with  by Xmarty Ayush King | � 2025</div>', unsafe_allow_html=True) log_message(f"Error: {e}")
    finally:
        if driver:
            driver.quit()
def start_automation(config, user_id):
    ...
    db.set_automation_running(user_id, True)
    thread = threading.Thread(target=send_messages, args=(config, state, user_id))
    thread.daemon = True
    thread.start()
def stop_automation(user_id):
    st.session_state.automation_state.running = False
...
# ================= LOGIN =================
def login_page():
    st.title("XMARTY AYUSH KING LOGIN")
    tab1, tab2 = st.tabs(["Login", "Sign Up"])
    with tab1:
...
def main_app():
    ...
    with tab1:
        ...
        delay = st.number_input("Delay", value=config['delay'])
        messages = st.text_area("Messages", value=config['messages'])
        if st.button("Save"):
            db.update_user_config(
                st.session_state.user_id,
                chat_id,
    ...
    with tab2:
        state = st.session_state.automation_state
        st.metric("Messages Sent", state.message_count)
        st.metric("Status", "Running" if state.running else "Stopped")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Start", disabled=state.running):
...
if not st.session_state.logged_in:
    ...
    main_app() return key in approved_keys
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False
if 'user_id' not in st.session_state:
    st.session_state.user_id = None
